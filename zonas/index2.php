                ...
                <form action="<?= $modo_edicion ? 'actualizar.php' : 'guardar.php' ?>" method="POST">
                    ...
                    <div class="form-row-zonas">
                        ...
                        <div class="form-group-zona">
                            ...
                            <input type="text" id="nombre" name="nombre" value="<?= htmlspecialchars($nombre) ?>" placeholder="Ej: Salón Social" required>
                        </div>

                        <!-- Campo: Descripción -->
                        <div class="form-group-zona" style="flex:1.5">
                            <label for="descripcion">Descripción</label>
                            <input type="text" id="descripcion" name="descripcion" value="<?= htmlspecialchars($descripcion) ?>" placeholder="Descripción breve" required>
                        </div>

                        <!-- Campo: Capacidad -->
                        <div class="form-group-zona" style="max-width:120px">
                            <label for="capacidad">Capacidad</label>
                            <input type="number" id="capacidad" name="capacidad" value="<?= $capacidad ?>" placeholder="50" required min="1">
                        </div>

                        <!-- Campo: Horario Disponible -->
                        <div class="form-group-zona">
                            <label for="horario">Horario Disponible</label>
                            <input type="text" id="horario" name="horario_disponible" value="<?= htmlspecialchars($horario) ?>" placeholder="Ej: 08:00 - 22:00" required>
                        </div>

                        <!-- Botón: Guardar -->
                        <div class="form-group-zona" style="flex:0 0 auto">
                            <label>&nbsp;</label> <!-- Espaciado para alinear con inputs -->
                            <button type="submit" class="btn-guardar-zona">
                                <?= $modo_edicion ? 'Actualizar Zona' : 'Guardar Zona' ?>
                            </button>
                            <?php if ($modo_edicion): ?>

                                <a href="index.php">
                                    <button type="button" class="btn-cancelar-zona">
                                        Cancelar
                                    </button>
                                </a>
                            <?php endif; ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="acciones-zonas">
                                        ...
                                        <a href="index.php?editar=<?= $fila['id'] ?>" class="btn-accion btn-editar">Editar</a>
